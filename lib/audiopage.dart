...
class AudioPage extends StatefulWidget {
  ...
  _AudioPageState createState() =>
      _AudioPageState(this.channelName, this.token, this.rtmToken, this.sec10);
}
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Audio Feed", style: TextStyle(color: Styles.blackColor)),
          elevation: 0,
        ),
        body: Stack(
          children: <Widget>[
            ...
            Column(
              ...
              children: [
                Center(
                  child: IconTheme(
                      data: new IconThemeData(color: login_bg),
                      child: IconButton(
                          onPressed: null,
                          iconSize: 100.0,
...
//Stop stream
  Future<bool> _stopStream() async {
    ...
    var response = await http.post(Uri.parse(url + '/stop_user_audio'),
  ...
  Future<bool> _stopStream10() async {
    ...
    var response = await http.post(Uri.parse(url + '/stop_user_audio10'),
        body: data, headers: headers);
    var jsonData = jsonDecode(response.body);
    print(jsonData);
    if (response.statusCode == 200) {
